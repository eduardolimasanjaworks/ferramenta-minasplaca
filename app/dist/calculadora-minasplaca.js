...
export function definirProdutos(novos) {
    produtos = [...novos];
}
export function listarProdutos() {
    return [...produtos];
}
export function calcularOrcamento(solicitacao) {
    const itens = [];
    const observacoes = [];
    let total = 0;
    for (const s of solicitacao) {
        const produto = produtos.find((p) => p.nome.toLowerCase() === s.nome.toLowerCase() || p.sku.toLowerCase() === s.nome.toLowerCase());
        if (!produto) {
...
export function textoOrcamento(orcamento) {
    if (!orcamento.itens.length)
        return 'Nao consegui identificar os produtos. Pode reformular?';
    const linhas = orcamento.itens.map((i) => `- ${i.produto.nome}: ${i.quantidade} ${i.produto.unidade} x R$ ${i.produto.precoUnitario.toFixed(2)} = R$ ${i.total.toFixed(2)}`);
