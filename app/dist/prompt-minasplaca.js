...
export async function inicializarBancoPrompt() {
    await pool.query(`
  ...
  `);
    ...
    if (existe.rowCount === 0) {
        await pool.query('INSERT INTO configuracao (chave, valor) VALUES ($1, $2)', ['prompt_sistema', PROMPT_PADRAO]);
    }
}
export async function obterPromptBruto() {
    const res = await pool.query('SELECT valor FROM configuracao WHERE chave = $1', ['prompt_sistema']);
    return res.rows[0]?.valor ?? PROMPT_PADRAO;
}
export async function salvarPrompt(prompt) {
    await pool.query(`INSERT INTO configuracao (chave, valor, atualizado_em)
     VALUES ($1, $2, NOW())
     ON CONFLICT (chave) DO UPDATE SET valor = $2, atualizado_em = NOW()`, ['prompt_sistema', prompt]);
}
