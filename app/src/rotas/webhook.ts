...
import { salvarEstadoMonitorTelefone } from '../servicos/monitor-telefone.js';
      ...
      try {
        ...
        if (!podeResponder) {
          ...
          await salvarEstadoMonitorTelefone(telefone, {
            fase: 'concluido',
            mensagem: 'Mensagem recebida sem resposta automatica',
            desdeMs: Date.now(),
            detalhe: 'contato pausado ou nao liberado para responder',
          }).catch(() => undefined);
          logEvento('webhook', 'Contato pausado — inbound registrado sem resposta', {
            telefone,
            tipo,
            remoteJid,
          });
