...
function mensagensJornada(
  ...
  if (jornada.cenario === 0) {
    ...
    return [
      ...
      driver(`jr_${jornada.id}_4`, '09:00:29', 'Vou mandar a CNH agora', 'Motorista parceiro'),
      assistant(`jr_${jornada.id}_5`, '09:00:31', msgs.c8_confirmacao_cnh, 'O proximo passo operacional continua objetivo depois do motorista escolher um documento pendente', 'Fluxo esperado: receber anexo da CNH -> confirmar -> seguir para CRLV do cavalo.', [act('cnh', 'upsert', '09:00:31', { status: 'recebida' }, 'CNH salva para validacao')], ['OCR', 'Cadastro']),
    ];
  }
  if (jornada.cenario === 1) return [assistant(`jr_${jornada.id}_1`, '09:00:00', jornada.mensagemPadrao, 'Pedido real de comprovante/canhoto', `Fluxo real usa a mensagem operacional atual.\n${msgs.canhoto_pedir_foto}`, [], ['Catalogo', 'Canhoto']), driver(`jr_${jornada.id}_2`, '09:00:10', 'Segue a foto do comprovante de entrega', 'Motorista parceiro'), assistant(`jr_${jornada.id}_3`, '09:00:13', msgs.canhoto_ok.replace('{{embarque_id}}', '8451'), 'Confirmacao operacional apos vincular o canhoto ao embarque', 'Fluxo real: gravar comprovante -> validar embarque -> confirmar ao motorista.', [act('delivery_receipts', 'insert', '09:00:13', { embarque_id: 8451, arquivo: 'canhoto.jpg' }, 'canhoto vinculado ao embarque')], ['ERP', 'Canhoto']), driver(`jr_${jornada.id}_4`, '09:00:28', 'Precisa de mais alguma coisa?', 'Motorista parceiro'), assistant(`jr_${jornada.id}_5`, '09:00:31', 'Nao parceiro, comprovante certo por aqui, qualquer novidade te chamo', 'Depois do canhoto validado o fluxo encerra sem escalar humano', 'Fluxo real: embarque atualizado -> conversa encerrada de forma curta.', [], ['Encerramento'])];
  if (jornada.cenario === 5) return [assistant(`jr_${jornada.id}_1`, '09:00:00', jornada.mensagemPadrao, 'Oferta proativa carregada do catalogo atual', 'Fluxo real usa a rota e o valor ofertado configurados no disparo.', [act('historico_ofertas', 'insert', '09:00:00', { config_rota_id: 301, motorista_id: 912, valor_ofertado: 4500 }, 'oferta_disparada_ia')], ['Catalogo', 'Oferta']), driver(`jr_${jornada.id}_2`, '09:00:09', 'Consigo por 4700', 'Motorista parceiro'), assistant(`jr_${jornada.id}_3`, '09:00:12', 'Entendi os R$ 4.700 parceiro, o maximo pra essa rota e R$ 4.600, topa nesse valor?', 'Contraproposta deterministica dentro da faixa da rota selecionada', 'Fluxo real esperado: oferta vinculada a config_rota_id + piso/teto do ERP + contraproposta em codigo.', [], ['Negociacao', 'config_rotas']), driver(`jr_${jornada.id}_4`, '09:00:27', 'Fecho em 4600', 'Motorista parceiro'), assistant(`jr_${jornada.id}_5`, '09:00:30', 'Boa parceiro, fechamos em R$ 4.600, ja deixei seu interesse registrado aqui', 'Aceite final registrado sem depender do LLM', 'Fluxo real: aceite dentro da faixa -> resposta_oferta_carga -> historico_ofertas.', [act('historico_ofertas', 'update', '09:00:30', { config_rota_id: 301, valor_aceito: 4600, aceite: 1 }, 'aceite_negociado_ia')], ['ERP', 'Aceite'])];
  if (jornada.cenario === 7) return [assistant(`jr_${jornada.id}_1`, '09:00:00', jornada.mensagemPadrao, 'Abertura real do fluxo programatico de disponibilidade', `Mensagens operacionais atuais: ${msgs.c7_pergunta_status}`, [], ['Catalogo', 'Disponibilidade']), driver(`jr_${jornada.id}_2`, '09:00:08', 'Ainda to carregado parceiro', 'Motorista parceiro'), assistant(`jr_${jornada.id}_3`, '09:00:11', msgs.c7_pergunta_local_atual_carregado, 'Fluxo continua sem LLM para coletar local atual', 'Sequencia real: status carregado -> local atual -> destino atual -> data de liberacao.', [], ['Programatico']), driver(`jr_${jornada.id}_4`, '09:00:24', 'To em Betim MG', 'Motorista parceiro'), assistant(`jr_${jornada.id}_5`, '09:00:28', msgs.c7_pergunta_destino_atual_carregado, 'Depois do local atual o fluxo avanca para destino atual do carregamento', 'Fluxo real segue coletando os campos necessarios em ordem fixa.', [], ['Programatico']), driver(`jr_${jornada.id}_6`, '09:00:42', 'Descarrego em Uberlandia MG amanha cedo', 'Motorista parceiro'), assistant(`jr_${jornada.id}_7`, '09:00:46', msgs.c7_pergunta_data, 'Agora a IA pede quando o motorista libera para voltar a carregar', 'Fluxo real fecha data prevista antes de registrar o local de disponibilidade.', [act('disponibilidade_motorista', 'upsert', '09:00:46', { status: 'carregado', localizacao_atual: 'Betim MG', local_destino_atual: 'Uberlandia MG' }, 'parcial salvo no ERP')], ['ERP', 'Disponibilidade']), driver(`jr_${jornada.id}_8`, '09:01:01', 'Libero amanha 08h em Uberlandia MG', 'Motorista parceiro'), assistant(`jr_${jornada.id}_9`, '09:01:05', msgs.c7_pergunta_local_disponibilidade, 'Ultimo passo operacional para gravar a cidade onde o motorista ficara disponivel', 'Fluxo real fecha local de disponibilidade e conclui a escrita no ERP.', [], ['Programatico'])];
  if (jornada.cenario === 8) return [assistant(`jr_${jornada.id}_1`, '09:00:00', jornada.mensagemPadrao, 'Inicio real do cadastro programatico', `Fluxo C8 atual.\n${msgs.c8_inicio}`, [], ['Catalogo', 'Cadastro']), driver(`jr_${jornada.id}_2`, '09:00:12', 'Segue a CNH', 'Motorista parceiro'), assistant(`jr_${jornada.id}_3`, '09:00:15', msgs.c8_confirmacao_cnh, 'Confirmacao do primeiro passo de cadastro apos receber CNH', 'Fluxo real: CNH -> CRLV -> ANTT -> endereco -> foto do cavalo.', [act('cnh', 'upsert', '09:00:15', { status: 'recebida' }, 'CNH salva para validacao')], ['OCR', 'Cadastro']), driver(`jr_${jornada.id}_4`, '09:00:31', 'Mandei o CRLV tambem', 'Motorista parceiro'), assistant(`jr_${jornada.id}_5`, '09:00:34', 'CRLV recebido parceiro, agora manda a ANTT do cavalo por favor', 'Fluxo continua passo a passo sem pular documento obrigatorio', 'Fluxo real: cada documento confirmado destrava o proximo pedido.', [act('crlv_cavalo', 'upsert', '09:00:34', { status: 'recebido' }, 'CRLV salvo para validacao')], ['OCR', 'Cadastro']), driver(`jr_${jornada.id}_6`, '09:00:52', 'Beleza, vou mandar a ANTT', 'Motorista parceiro')];
  return [assistant(`jr_${jornada.id}_1`, '09:00:00', jornada.mensagemPadrao, 'Mensagem inicial real do catalogo persistido', ['Origem: catalogo-jornadas-teste', `Jornada: ${jornada.id}`, `Cenario: ${jornada.cenario}`, '', jornada.mensagemPadrao].join('\n'), [], ['Catalogo', 'Backend real']), driver(`jr_${jornada.id}_2`, '09:00:09', 'Beleza parceiro', 'Motorista parceiro'), assistant(`jr_${jornada.id}_3`, '09:00:12', 'Perfeito, me fala se voce quer cadastro, frete ou pagamento pra eu seguir certo por aqui', 'Continuacao mais util para menu e saudacao inicial', 'Fluxo esperado: acolher a saudacao curta e pedir o assunto principal sem abrir menu engessado.', [], ['Menu']), driver(`jr_${jornada.id}_4`, '09:00:26', 'Quero saber de frete', 'Motorista parceiro'), assistant(`jr_${jornada.id}_5`, '09:00:29', 'Boa, me fala origem e destino que eu vejo o que faz sentido pra voce', 'Saudacao vira triagem operacional em vez de resposta vazia', 'Fluxo real esperado: identificar se o contato quer frete, cadastro ou pagamento e seguir o cenario correto.', [], ['Triagem'])];
}
