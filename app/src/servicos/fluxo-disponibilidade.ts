...
const DATA_VAGA =
  'Preciso de uma estimativa de data parceiro, que dia acha que libera?';

const FECHAMENTO = 'Show parceiro, dados atualizados, boa viagem';
...
interface EstadoC7 {
  passo: 'status' | 'vazio_local' | 'carregado_destino' | 'carregado_data';
  destino?: string;
}
...
type ContextoC7 =
  ...
  | { tipo: 'carregado_destino' }
  | { tipo: 'carregado_data'; destino: string };
...
function perguntouDestino(texto: string): boolean {
  return /cidade.*estado.*indo|pra qual cidade/i.test(texto);
}
...
function ehEntradaDisponibilidade(mensagem: string): boolean {
  const t = normalizar(mensagem);
  return /^disponibilidade$/.test(t) || /^quero\s+disponibilidade$/.test(t);
}
...
function inferirContexto(
  ...
  if (ehEntradaDisponibilidade(mensagem)) return { tipo: 'entrada' };

  if (perguntouData(ultimaAssist)) {
    const destino = extrairDestinoDoHistorico(historico) ?? '';
    return { tipo: 'carregado_data', destino };
  }

  if (perguntouDestino(ultimaAssist)) return { tipo: 'carregado_destino' };
...
function extrairDestinoDoHistorico(
  historico: Array<{ role: string; content: string }>,
): string | null {
...
export async function tentarFluxoDisponibilidade(opts: {
  ...
  if (!contexto && estadoRedis) {
    ...
    if (estadoRedis.passo === 'carregado_destino') contexto = { tipo: 'carregado_destino' };
    if (estadoRedis.passo === 'carregado_data' && estadoRedis.destino) {
      contexto = { tipo: 'carregado_data', destino: estadoRedis.destino };
    }
  ...
  if (contexto.tipo === 'entrada') {
    ...
    return montarResultado(PERGUNTA_STATUS, undefined, 'pergunta_status');
  }

  if (contexto.tipo === 'aguardando_status') {
    if (ehRespostaAmbiguaStatus(mensagem) || (!ehVazio(mensagem) && !ehCarregado(mensagem))) {
      return montarResultado(DUVIDA_STATUS, undefined, 'duvida_status');
    }
    if (ehVazio(mensagem)) {
      await salvarEstadoFluxo(telefone, { passo: 'vazio_local' } satisfies EstadoC7);
      return montarResultado(PEDE_LOCALIZACAO, undefined, 'pede_local');
    }
    if (ehCarregado(mensagem)) {
      await salvarEstadoFluxo(telefone, { passo: 'carregado_destino' } satisfies EstadoC7);
      return montarResultado(PERGUNTA_DESTINO, undefined, 'pede_destino');
    }
  }

  if (contexto.tipo === 'vazio_localizacao') {
    if (localizacaoVaga(mensagem)) {
      return montarResultado(LOCAL_INVALIDA, undefined, 'local_invalida');
    }
  ...
  if (contexto.tipo === 'carregado_destino') {
    const destino = extrairLocalizacaoTexto(mensagem);
    if (!destino || localizacaoVaga(mensagem)) {
      return montarResultado(PERGUNTA_DESTINO, undefined, 'destino_invalido');
    }
    await salvarEstadoFluxo(telefone, { passo: 'carregado_data', destino } satisfies EstadoC7);
    return montarResultado(PERGUNTA_DATA, undefined, 'pede_data');
  }

  if (contexto.tipo === 'carregado_data') {
    if (dataVaga(mensagem)) {
      return montarResultado(DATA_VAGA, undefined, 'data_vaga');
    }
    const dataIso = parseDataLiberacao(mensagem);
    if (!dataIso) {
      return montarResultado(DATA_VAGA, undefined, 'data_invalida');
    }
    ...
    return montarResultado(
      FECHAMENTO,
      {
        ...
        dados: {
          ...
          data_previsao_disponibilidade: dataIso,
